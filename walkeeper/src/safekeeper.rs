...
/// announces 1) successfull election (with VCL); 2) commit_lsn.
#[derive(Debug, Serialize, Deserialize)]
...
impl ProposerAcceptorMessage {
    ...
    pub fn parse(msg: Bytes) -> Result<ProposerAcceptorMessage> {
        ...
        match tag {
            'g' => {
                let msg = des_stream_le::<ProposerGreeting, Reader<Bytes>>(&mut stream)?;
                Ok(ProposerAcceptorMessage::Greeting(msg))
            }
            'v' => {
                let msg = des_stream_le::<VoteRequest, Reader<Bytes>>(&mut stream)?;
                Ok(ProposerAcceptorMessage::VoteRequest(msg))
            }
            'a' => {
                // read header followed by wal data
                let hdr = des_stream_le::<AppendRequestHeader, Reader<Bytes>>(&mut stream)?;
                let rec_size = hdr.end_lsn.checked_sub(hdr.begin_lsn).unwrap().0 as usize;
